...
class MERAState:
        ...
        while len(basis) > 1:
            ...
            for i in range(n_sites):
                ...
                if max_bond is not None:
                    info_high.truncate_no_keep(max_bond)
                if n_sites == 1:
                    info_high = info_high.filter(
                        info_high.__class__({target: 1}))
                ts = SparseFermionTensor.random(
    ...
    def unitize_(mera):
        for t in mera:
            if t.data.ndim - len(t.left_inds) == 1:
                ...
                fuse_info = t.data.kron_sum_info(
                    2, 3, pattern=t.data.pattern[2:])
                new_data = t.data.fuse(2, 3, info=fuse_info).left_canonicalize()[
                    0].unfuse(2, info=fuse_info)
    ...
    def get_hamil_terms(topology, oyb_sym, basis, dtype=float):
        ...
        for (i, j, v) in topology:
            if i != j:
                ts = sum([
                    np.tensordot(v * ca_op[i], da_op[j],
                                 axes=([], [])).transpose((0, 2, 1, 3)),
                    np.tensordot(-v * da_op[i], ca_op[j],
                                 axes=([], [])).transpose((0, 2, 1, 3)),
                    np.tensordot(v * cb_op[i], db_op[j],
                                 axes=([], [])).transpose((0, 2, 1, 3)),
                    np.tensordot(-v * db_op[i], cb_op[j],
                                 axes=([], [])).transpose((0, 2, 1, 3))
                ])
                terms.append(((i, j), ts))
            else:
...
class TensorBuilder:
    def __init__(self, hamil, max_bond=None):
        self.uni, self.iso, self.cap = MERAState.get_disentanglers_and_isometries(
            hamil.basis, hamil.target, max_bond=max_bond)
        self.i_uni = 0
        self.i_iso = 0
        self.i_cap = 0
...
def local_expectation(mera, terms, where, optimize='auto-hq'):
    ...
    fsites = list(mera_ij.filled_sites) + \
        list(set(mera.filled_sites) - set(mera_ij.filled_sites))
    tid_map = {mera.fermion_space.get_tid_from_site(
        x): ix for ix, x in enumerate(fsites)}
...
if __name__ == '__main__':
    ...
    mera = MERA.from_fill_fn(
        ...
        # number of sites, (doesn't have to be a power of block_size)
        L=L,
    ...
    for ts in mera:
        new_ts = IsoFermionTensor(
            data=ts.data, inds=ts.inds, tags=ts.tags, left_inds=ts.left_inds)
